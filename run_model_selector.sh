...
for exp_root in "$ROOT"/outputs/jobs_monotone_spatioTempTransformer_spatialLoss/*-livneh ; do
  [[ -d "$exp_root" ]] || continue
  model="$(basename "$exp_root")"  # e.g., gfdl_esm4-gridmet
  outdir="$ROOT/outputs/jobs_monotone_spatioTempTransformer_spatialLoss/$model"
  mkdir -p "$outdir"
